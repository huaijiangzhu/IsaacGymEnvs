...
def _proj_friction_cone(forces: torch.Tensor, friction_coeff: torch.Tensor):
    ...
    '''
    batch_size, num_vars = forces.shape

    # reshape the forces so that each row has only one 3D force
    b = int(batch_size * num_vars / 3)
    ...
    return projected_forces.view(batch_size, num_vars)
...
class FISTA(torch.nn.Module):
    ...
    def __init__(self, prob, step_size=None, device=torch.device("cuda:0")):
        ...
        if step_size is not None:
            self.step_size = step_size * torch.ones(self.prob.batch_size).to(self.device)
        # jitted functions
    ...
    def compute_step_size(self):
        ...
        return step_size.view(self.prob.batch_size, 1)
...
class QP(object):
    @torch.no_grad()
    def __init__(self, batch_size, num_vars, num_eqc, friction_coeff=None, device=torch.device("cuda:0")):
        self.device = device
        ...
        if isinstance(friction_coeff, torch.Tensor):
            self.friction_coeff = friction_coeff.to(device).view(batch_size, 1)
        elif isinstance(friction_coeff, float):
            self.friction_coeff = friction_coeff * torch.ones(batch_size, 1).to(device)
        else:
        ...
        self.Pk = torch.zeros(self.batch_size, self.num_eqc).to(self.device)
        self.ydiff = torch.zeros(self.batch_size, self.num_vars).to(self.device)
...
class ForceQP(object):
    @torch.no_grad()
    def __init__(self, batch_size, num_vars, friction_coeff=None, device=torch.device("cuda:0")):
        self.device = device
